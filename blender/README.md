# Introduction
This directory contains the Blender files used in constructing the front garden railway.
...
The `track-side_path_track_wall.stl`should be printed directly in ASA, 15% in-fill, no supports required, brim advisable to stop the print leaving the print-bed, and go for relatively high detail since this wall will be visible.  With such large, flat, ASA parts you will likely need glue-stick on the print bed to prevent the part warping away from it and you may need to consider printing the parts one at a time to avoid them applying enough force to pull the magnetic print plate away from its mount.  `track-side_path_track_wall_end.stl` should be printed in the same way. `track-side_path_track_wall_wedge.stl` doesn't require the detail, 10% in-fill, no supports required, brim advisable.
...
## `spiral_staircase.blend`
A feature to hide the monolithic lake-end of the "Rock Wall" of the track-side path (see above).  The Blender file contains a spiral staircase step to which an Array modifier has been added to form a spiral; however the modifier should NOT be applied, it is only there to show that the step works in a spiral.  Instead the single step is printed alone (exported `.stl` files at 30.48 Blender scale included) multiple times because that way it is actually printable without needing a tonne of support material.  20&nbsp;steps make a complete 360&nbsp;degree turn and they should be held together by a length of 1.5&nbsp;mm diameter (e.g. brass) bar slotted down the hole in the middle and anchored in the slot provided in the two-part circular base.  There is also a connector that attaches at the top of the staircase and a decorative weather vane that fits on top.  In addition, `spiral_staircase_top_platform*.stl` are the components of a viewing platform that the spiral staircase serves.

The steps and connector should be printed in black ASA at fastest speed, e.g. 0.2&nbsp;mm layer height, since there is no detail here to speak of, with 10% in-fill, supports everywhere, brim advisable.

Everything else apart from `spiral_staircase_top_platform*.stl` should also be printed in black ASA but at higher resolution (e.g. 0.1&nbsp;mm layer height) to capture the detail, supports everywhere, no brim required.  The weather vane N/S/E/W indicators are best cut in your printer's slicer program vertically through the longest axis to give a good printing surface, the parts glued together afterwards with cyanoacrylate adhesive; see the pictures [here](https://www.meades.org/railways/garden/garden.html#Fixing_The_Monolith) and [here](https://www.meades.org/railways/garden/garden.html#Viewing_Platform_Improvements).
...
In addition, there is a wall, with the same Array and Curve modifiers applied.  This is meant to be printed directly in a flexible material (it is too small to mould well).  The merging process of the Array modifier will leave internal structures so, if you modify it, best to Save As the Blender file (to avoid accidentally over-writing the original), apply the modifiers then tidy up the mesh.

`.stl` exports of all components are provided but if you modify any of them you will need to re-export from Blender at a Blender scale factor of 30.48.  The mould should be printed with no supports, 5% in-fill and at maximum print speed in any filament you like (PLA will probably be cheapest).  The wall I printed with 15% in-fill, highest resolutoin (since there is detail in the wall) and it will need to be cut into sections in your printer's slicer program for printing (with dovetail joints if you can) and then, rather dealing with support structures, I chose to further cut each section in half thickness-wise, place the pieces on their cut side and glue them together with cyanoacrylate adhesive afterwards.

See [here](https://www.meades.org/railways/garden/garden.html#Woodland_Path) for the moulding process and [here](https://www.meades.org/railways/garden/garden.html#A_Flexible_Wall) for the addition of the wall.

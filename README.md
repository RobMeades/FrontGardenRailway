...
## `filter_wall.blend`
A printable stone wall (plus `.stl` exports at 30.48 scale) that forms a filter for the water outlets of the lake.  This should be 3D printed in ASA for UV-safety, supports on the print bed only, 10% in-fill and I chose relatively high detail, since this is not going to be moulded, it needs to look sufficiently realistic in its plastic form; I found some matt grey ASA filament to print in.  Multiple copies of the parts will need to be printed, the upper and lower portions glued together then heated and bent to form a wall of the right shape before being glued together, with the back-strips offset, to join them and keep that shape.
